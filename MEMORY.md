...
### 2026-07-10: 逻辑说明增强 - 添加创建步骤、使用场景、效果示例
- **核心问题**: 用户不知道特征仓库和预测模型是干嘛的，也不知道怎么用
- **解决方案**: 重写逻辑说明弹窗，添加7个部分：
  1. 什么是XXX？（大白话解释）
  2. 在AIOps中哪里用？（使用场景表格）
  3. 创建/添加步骤（详细操作流程）
  4. 模型类型/实体类型说明（参数表格）
  5. 参数说明（window/degree/threshold）
  6. 预测结果解读（字段含义）
  7. 实际效果示例（具体场景+效果+行动）

### 2026-07-10: 预测引擎打通 - 支持5种模型类型
- **核心问题**: 预测模型只是配置存储，没有真正执行预测
- **解决方案**:
  1. prediction_service支持linear/arima/polynomial/rolling_avg/prophet五种模型
  2. 新增predict_with_model()函数，使用模型配置执行预测
  3. 新增API: /api/{mid}/predict 单模型预测, /api/predict-all 全部预测
  4. 预测结果包含: 当前值、趋势、R²、特征数据、未来48小时预测值
  5. 特征仓库数据作为预测上下文输入(cpu_avg_1h, mem_avg_1h等)
- **验证**: cpu_usage预测成功，返回当前值54.32，趋势上升，48个预测点

### 2026-07-10: 特征仓库和预测模型 - 添加逻辑说明和分页
- **核心问题**: 用户不知道特征仓库和预测模型是干嘛的，且无法翻页查看历史数据
- **解决方案**:
  1. 后端API支持page和page_size参数
  2. 前端添加el-pagination分页组件
  3. 新增"逻辑说明"弹窗，用白话解释功能用途、操作流程、参数说明
- **特征仓库说明**: ML模型可复用特征存储，用于异常检测、趋势预测、容量规划
- **预测模型说明**: 时序预测模型，支持线性回归、多项式、移动平均三种类型

### 2026-07-10: 智能体审计分页 — 支持翻页和每页条数选择
- **核心问题**: 审计记录只能显示前100条，无法查看历史记录
- **解决方案**: 
  1. 后端API支持page和page_size参数
  2. 前端添加el-pagination组件，支持20/50/100条每页
- **验证**: 测试分页API，返回total=134，page=1, page_size=5时返回5条记录

### 2026-07-10: Runbook严格执行规则 — 禁止AI自作主张替换命令
- **核心问题**: AI执行Runbook时自作主张，把`systemctl status nginx`换成`ps aux | grep nginx`等一堆命令
- **解决方案**: 系统提示词新增"Runbook严格执行规则"，明确：
  - 有Runbook时必须严格按步骤执行
  - 禁止自作主张添加额外诊断命令
  - 只有命令执行失败时才根据错误信息决定是否额外诊断
- **验证**: 重启后测试"重启39.96.51.45上的nginx"，AI正确执行`systemctl status nginx`检查状态

### 2026-07-10: query_assets搜索修复 — 支持tags和IP匹配
- **核心问题**: query_assets的search只匹配name，加了web标签的39.96.51.45搜不到
- **解决方案**: search字段现在同时匹配name、ip、tags三个字段
- **验证**: 搜"web"能正确返回8个资产，包含39.96.51.45(云主机, tags=web)

### 2026-07-10: 资产类型认知修复 — 云主机/虚拟机都是服务器
- **核心问题**: AI助手查服务器只查ci_type=server，漏掉了cloud_host(云主机)和vm(虚拟机)
- **解决方案**: 
  1. 系统提示词新增"资产类型说明"，明确server/cloud_host/vm都是服务器
  2. query_assets工具描述更新，说明搜索主机时可不传ci_type
- **验证**: 现在查"所有服务器"能正确返回15台(9 server + 1 cloud_host + 5 vm)
