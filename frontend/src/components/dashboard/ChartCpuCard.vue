...
onMounted(async () => {
  ...
  try {
    ...
    const data = d.vm_metrics?.cpu_usage || []
    chart.setOption({
      ...
      series: [{ type: 'line', smooth: true, symbol: 'none', data: data.map(x => x.value), lineStyle: { color: '#818cf8', width: 2 }, areaStyle: { color: 'rgba(129,140,248,0.12)' } }],
    })
  } catch {}
})
