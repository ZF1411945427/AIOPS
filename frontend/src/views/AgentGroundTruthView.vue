<template>
  <div class="gt-page">
    ...
    <div class="toolbar">
      <select v-model="filterCategory" class="input" @change="loadTests">
    ...
    <div class="panel">
      <div class="panel-head">测试用例</div>
      <div class="panel-body">
        <div v-if="loading" class="loading-state">加载中...</div>
        <div v-else-if="!tests.length" class="empty-state">暂无用例</div>
        <div v-else class="table-wrap">
          <table class="table">
            <thead><tr><th>名称</th><th>分类</th><th>难度</th><th>问题</th><th>操作</th></tr></thead>
            <tbody>
              <tr v-for="t in tests" :key="t.id">
                ...
                <td>
                  ...
                  <button class="btn btn-sm btn-primary" @click="runTest(t)">执行</button>
    ...
    <div class="panel" v-if="runs.length">
      <div class="panel-head">最近执行记录</div>
      <div class="panel-body">
        <div class="table-wrap">
          <table class="table">
            <thead><tr><th>用例</th><th>模型</th><th>答案分</th><th>工具分</th><th>总分</th><th>延迟</th></tr></thead>
            <tbody>
...
const form = ref({ name: '', category: 'qa', question: '', expected_answer: '', expected_tools_str: '', difficulty: 'medium', tags: '' })
async function loadTests() {
  ...
  try {
    ...
    const [testRes, runRes, statsRes] = await Promise.all([
      ...
      request.get('/agent/api/ground-truth/runs', { params: { limit: 10 } }),
...
function openCreate() {
  editing.value = false
  form.value = { name: '', category: 'qa', question: '', expected_answer: '', expected_tools_str: '', difficulty: 'medium', tags: '' }
  showForm.value = true
}
function openEdit(t) {
  editing.value = true
...
async function submitForm() {
  try {
    const payload = {
      ...form.value,
      expected_tools: form.value.expected_tools_str ? form.value.expected_tools_str.split(',').map(s => s.trim()) : [],
    }
    delete payload.expected_tools_str
    if (editing.value) {
...
async function runTest(t) {
  try {
    const res = await request.post(`/agent/api/ground-truth/tests/${t.id}/run`)
    ElMessage.success(`执行完成 总分: ${res.run?.total_score}`)
    loadTests()
