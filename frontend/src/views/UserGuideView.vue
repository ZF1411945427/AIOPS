...
const guideGroups = [
  {
    ...
    num: '链路1',
    title: '核心数据流（完整闭环）',
    flowTags: ['资产发现', '指标采集', '可观测性', '异常检测', '告警', '故障单', '知识沉淀', 'AI检索', '自愈'],
    steps: [
      ...
      { path: '侧边栏 → 值班驾驶舱 → 故障管理 → 故障详情', action: '故障处理完成后，点击「解决」，故障单直接关闭', expect: '故障状态从「待处理」变为「已解决」' },
      { path: '侧边栏 → 知识库 → AI知识草稿（后台自动）', action: '无需操作，故障解决后系统自动沉淀知识', expect: 'AI知识草稿页面出现新草稿，状态为「待审批」' },
  ...
  },
  {
    ...
    steps: [
      { path: '侧边栏 → 值班驾驶舱 → 故障管理 → 故障详情', action: '故障处理完成后，点击「解决」关闭故障单，系统自动触发知识沉淀', expect: '后台调用LLM生成结构化知识（标题/症状/根因/解决方案/标签）' },
