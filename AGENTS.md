...
## ⚠️ 高危操作人工审核铁律(重要!)
**任何 AI(我)执行可能破坏数据/影响运行的高危操作前,必须在对话里先把完整命令展示给爸爸,并等爸爸明确同意后才能执行。**

覆盖所有高危场景,统一"删前/改前必审"规则:
1. **删除类**——本机/远程/部署脚本里的 `rm`/`rm -rf`、SQL 的 `DELETE`/`DROP`、批量清理、临时文件清理
2. **写坏类**——`DROP TABLE`/`TRUNCATE`、`git reset --hard`/`push --force`、`rm` 之外的 `mv` 覆盖、配置文件大面积改写
3. **运维类**——重启/停服务(尤其生产)、防火墙/网络改动、`systemctl disable`、Docker 删镜像/容器
4. **数据类**——改生产库数据、清空表、覆盖他人/他机文件

统一要求:
1. **绝不静默执行**——先输出完整命令 + 影响对象清单 + 影响范围,询问确认
2. 能安全替代就替代:改名备份(`mv 目录 目录.bak`)而非 `rm -rf`;`touch` 占位;幂等判断(`[ -f x ] && ...`);先 dry-run/预览
3. 脚本里必须含高危操作时,先展示脚本片段再执行
4. 用户没说"可以了/执行吧"就不动;一次性批量高危险操作要逐个点明
5. 涉及生产数据、虚机文件、他人目录的高危险操作,绝对要人等

**背景教训:** 之前 Kafka 部署脚本里 `rm -rf $KAFKA_HOME` 配合引号/变量问题,把虚机 `/data` 下 redis/harbor 等多个目录一起误删,只能靠快照回滚或接受损失。所以"高危必审"是硬性红线。
